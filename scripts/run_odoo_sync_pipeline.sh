...
run_step \
    ...
    bash "$SCRIPT_DIR/transform_base_to_splited.sh" \
    "$SOURCE_ROOT" \
    "$SPLITED_TARGET"

run_step \
    "Transform splited to compile_left" \
    bash "$SCRIPT_DIR/transform_splited_to_left.sh" \
    "$SPLITED_TARGET" \
    "$LEFT_TARGET"

run_step \
    "Compile models for all configured Python versions" \
    bash "$SCRIPT_DIR/compile_models_all_versions.sh" \
    "$LEFT_TARGET" \
    "$COMPILED_TARGET"

run_step \
    "Delete runtime model methods into final target" \
    bash "$SCRIPT_DIR/delete_model_methods.sh" \
    "$COMPILED_TARGET" \
    "$FINAL_TARGET"
