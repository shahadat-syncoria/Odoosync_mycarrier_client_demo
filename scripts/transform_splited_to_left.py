...
def rewrite_imports(root: Path) -> int:
    ...
    for path in sorted(root.rglob("*.py")):
        if should_skip_runtime_scan(path):
            continue
...
def should_skip_runtime_scan(path: Path) -> bool:
    return "static" in path.parts or "scripts" in path.parts
...
def find_bad_runtime_inherit(root: Path) -> list[str]:
    bad = []
    for path in sorted(root.rglob("*.py")):
        if "table_models" in path.parts or should_skip_runtime_scan(path):
            continue
